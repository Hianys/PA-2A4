<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold">@lang("Edit the announcement")</h2>
    </x-slot>
    ...
    <div class="max-w-3xl mx-auto py-6 space-y-8">
        ...
        <div class="bg-white shadow rounded-lg p-6">
            <h3 class="text-lg font-semibold mb-4">@lang("Edit the announcement")</h3>

            <form method="POST" action="{{ route('provider.annonces.update', $annonce) }}" enctype="multipart/form-data" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="type" id="type" onchange="toggleFields()" class="mt-1 block w-full rounded border-gray-300">
                        <option value="transport" {{ old('type', $annonce->type) == 'transport' ? 'selected' : '' }}>@lang("Transport")</option>
                        <option value="service" {{ old('type', $annonce->type) == 'service' ? 'selected' : '' }}>@lang("Service")</option>
                    </select>
                ...
                <div id="transport-fields" class="space-y-4" style="{{ old('type', $annonce->type) === 'transport' ? '' : 'display: none;' }}">
                    ...
                    <div>
                        <label for="weight" class="block text-sm font-medium text-gray-700">@lang("Weight") (kg)</label>
                    ...
                    <div>
                        <label for="volume" class="block text-sm font-medium text-gray-700">@lang("Volume") (m³)</label>
